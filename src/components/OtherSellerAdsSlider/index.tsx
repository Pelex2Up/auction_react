...
import { useFetchLastVisitedMutation, useFetchUserAdvertisementsQuery } from '../../api/userService'

interface IUserAdsSlider {
  userId: number | string
}

export const OtherSellerAdsSlider: FC<IUserAdsSlider> = ({ userId }) => {
  const { language } = useAppSelector(selectLangSettings)
  const { lots: lotsHistory } = useAppSelector(selectHistory)
  const { data, isSuccess, isLoading, isError, refetch } = useFetchUserAdvertisementsQuery(userId)
      ...
      <div className="self-stretch justify-between items-center inline-flex">
        <div className="w-[372px] h-[29px] pr-[7px] justify-start items-center flex">
          <div className="text-zinc-900 text-2xl font-medium font-['SF Pro Text'] leading-[28.80px] tracking-tight">
            {language === 'RU' ? 'Другие объявления пользователя' : 'Other advertisements from that seller'}
          </div>
        </div>
        ...
        >
          {lotsData?.map((lot, index) => (
            <SwiperSlide style={{ display: 'flex', justifyContent: 'center' }} key={index}>
              <AuctionCard lot={lot} refetch={() => refetch} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
