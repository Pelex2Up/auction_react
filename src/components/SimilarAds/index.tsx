...
import { useFetchLastVisitedMutation, useFetchSameLotsMutation } from '../../api/userService'

interface ISameBlock {
  id: number
}

export const SameLotsBlock: FC<ISameBlock> = ({ id }) => {
      ...
      <div className="self-stretch justify-between items-center inline-flex">
        <div className="w-[372px] h-[29px] pr-[7px] justify-start items-center flex">
          <div className="text-zinc-900 text-2xl font-medium font-['SF Pro Text'] leading-[28.80px] tracking-tight">
            {language === 'RU' ? 'Похожие объявления' : 'Similar advertisements'}
          </div>
        </div>
        ...
        >
          {lotsData?.map((lot, index) => (
            <SwiperSlide style={{ display: 'flex', justifyContent: 'center' }} key={index}>
              <AuctionCard lot={lot} refetch={() => getLots(id)} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
