...
export default function ProfileHeader({ ...props }: { username: string; avatar: string }) {
  ...
  const handleOutsideClick = useCallback(
    (event: MouseEvent) => {
      if (listRef.current && !listRef.current.contains(event.target as Node) && show) {
        setShow(false)
      }
    },
    [show]
  ...
  return (
    ...
    >
      ...
      >
        <img src={props.avatar ? props.avatar : userImage} className="w-[40px] h-[40px] rounded-full" alt="avatar" />
        <div
          style={{
            color: '#808080',
            fontSize: 14,
      ...
      {show && (
        <ul className={`absolute right-0 top-[110%] w-[260px] z-10 cursor-default`}>
